...
function connectDB(): Database {
  let db : Database = new sqlite3.Database(path.join(__dirname, '../database/database.db'),sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, (err : Error | null) : void =>{
    if(err) {
...
const requestHandler = (req:IncomingMessage,res:ServerResponse) : void => {
  ...
  if(req.method === 'GET'){
    ...
    db.all('SELECT name, explain, price FROM product', (err: Error | null, rows: Object[]): void => {
      if (err) {
      ...
      db.close((err: Error | null): void => {
        if (err) {
          console.error('데이터베이스 닫기 중 오류 발생:', err.message);
        } else {
